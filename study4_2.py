...
for mol in true_list:
    bit_info = {}
    X = Chem.RDKFingerprint(mol, bitInfo=bit_info)
...
"""
for bit in bits:
    print(selected_bit_info[0][bit])
"""
most_bit = cnt.most_common()[0][0]
print("most-bit:",most_bit)
target = selected_mols[2]
bit_info = selected_bit_info[2]
print("bits:", bits)
morgan_turples = ((target, bit, bit_info) for bit in bits)
print("selected_mols: ",len(selected_mols))
print("selected_bit_info: ", len(selected_bit_info))
print("target: ",target)
print("bit_info: ", len(bit_info))
img = Draw.DrawRDKitBits(morgan_turples, molsPerRow=5,  subImgSize=(400,400),legends=['rdkit-selected: '+str(x) for x in range(n_most_common)])
print(type(img))
with open('sample_rdkit.xml', mode='w') as f:
    f.write(img)
